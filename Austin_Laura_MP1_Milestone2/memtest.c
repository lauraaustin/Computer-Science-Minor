...
/*   // Testing the my_allocator function based off of PIAZZA post "How to test Milestone 1 of MP1" by Matthew Collie
  printf("\n%d", init_allocator(1,50)); // Should return 50
  printf("\n%x", my_malloc(7)); 		// Should return a hex val
  printf("\n%x", my_malloc(6));			// Should return a hex val + 7
  printf("\n%x", my_malloc(50));		// Should return Null or zero */
  
  // Add code to process input parameters (basic block size, memory length)
  int basic_block_size = 10;
  int memory_length = 2000000;
  // Initialize the allocator by calling: 
  init_allocator(basic_block_size, memory_length);

  ackerman_main();

  // Release the memory in the allocator by calling: 
  release_allocator();
